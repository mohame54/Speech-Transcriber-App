...
class AudioTranscriber(Resource):
    ...
    def post(self):
        ...
        if audio_file is None:
            return {"message":"could't find any audio files!"}  
        audio_file =  base64.b64decode(audio_file['content'])  
        decoding = generation_kwargs.get("decoding", None) 
        if decoding is not None:
            del generation_kwargs['decoding']     
        #audio_file.seek(0)
        audio_data, _ = load_wav(io.BytesIO(audio_file))
        # Transcribe the loaded wav
        hyps = inf(audio_data, decoding=decoding, **generation_kwargs)
